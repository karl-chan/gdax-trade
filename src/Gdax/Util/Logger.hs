        ...
        when (ll >= logLim) $ do
            ...
            let loc =
                    case GHC.getCallStack cs of
                        ...
                        _ -> "unknown"
                msg = "[" <> renderLevel <> "] " <> toLogStr loc <> " - " <> toLogStr txt <> "\n"
...
withGlobalLogging LogConfig {..} f = bracket initLogger flushLogger (const f)
  where
    ...
    initLogger = do
        fileLogger <-
            flip T.mapM logFile $ \fp -> do
                let spec = FileLogSpec {log_file = fp, log_file_size = 1024 * 1024 * 50, log_backup_number = 5}
