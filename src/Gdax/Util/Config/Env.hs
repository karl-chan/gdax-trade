...
getEnvConfig = do
    ...
    when (isNothing maybeKey) $ error "GDAX_KEY environmental variable is missing. Please see README.id"
    when (isNothing maybeSecret) $ error "GDAX_SECRET environmental variable is missing. Please see README.md"
    when (isNothing maybePassphrase) $ error "GDAX_PASSPHRASE environmental variable is missing. Please see README.md"
    ...
    when (isNothing maybeSandboxKey) $ error "GDAX_SANDBOX_KEY environmental variable is missing. Please see README.id"
    when (isNothing maybeSandboxSecret) $
        error "GDAX_SANDBOX_SECRET environmental variable is missing. Please see README.md"
    when (isNothing maybeSandboxPassphrase) $
        error "GDAX_SANDBOX_PASSPHRASE environmental variable is missing. Please see README.md"
    ...
    return
        ...
        , sandboxCredentials = EnvCredentialsConfig sandboxKey sandboxSecret sandboxPassphrase
        , server = EnvServerConfig maybeServerUser maybeServerPass port
        }
