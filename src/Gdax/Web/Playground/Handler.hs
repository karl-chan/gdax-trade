...
playgroundHandler = do
  ...
  model <-
    buildModel searchMethod maybeSearchArgs maybeBefore maybeAfter sandbox
...
buildModel (Just STREAM) (Just args) _ _ sandbox = do
  ...
  case safeReads args of
    Left err ->
      return
        Error {method = STREAM, args = args, msg = err, sandboxMode = sandbox}
    Right products -> do
      let socketUrl =
            "wss://" ++
            if sandbox
...
buildModel (Just method) (Just url) maybeBefore maybeAfter sandbox = do
  ...
  case res of
    SearchError err ->
      return
        Error
        {method = method, args = url, msg = show err, sandboxMode = sandbox}
    SearchResponse {..} ->
      return
        Rest
        { method = method
...
buildModel _ _ _ _ _ = error "Unsupported arguments in buildModel"
