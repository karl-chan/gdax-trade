...
simpleFragment Rest {..} = render method url pagination responseBody timeTakenMillis

render :: SearchMethod -> SearchUrl -> PaginationOptions -> ByteString -> Double -> H.Html
render method url pagination textContent timeTakenMillis =
    H.div ! A.class_ "output-card demo-card-wide mdl-card mdl-shadow--2dp" $ do
        H.div ! A.class_ "mdl-card__title output-title" $ do
            ...
            H.span ! A.class_ "time_taken" $ string $ "Took " ++ show timeTakenMillis ++ " ms."
            renderPagination method url pagination
        H.div ! A.class_ "mdl-card__supporting-text" $ H.pre ! A.class_ "output-text" $ return ()
        H.input ! A.type_ "hidden" ! A.name "response_text" ! A.value (stringValue . cs $ textContent)
        H.script ! A.type_ "text/javascript" ! A.defer "" ! A.src (stringValue $ jsFile "playground/search.js") $
            return ()

renderPagination :: SearchMethod -> SearchUrl -> PaginationOptions -> H.Html
renderPagination method url pagination =
    H.form ! A.class_ "pagination_form" ! A.enctype "multipart/form-data" ! A.method "POST" ! A.action "/playground" $ do
        H.input ! A.type_ "hidden" ! A.name "search_method" ! A.value (stringValue . show $ method)
        H.input ! A.type_ "hidden" ! A.name "search_args" ! A.value (stringValue url)
