...
render products url initialMessage =
    H.div ! A.class_ "output-card demo-card-wide mdl-card mdl-shadow--2dp" $ do
        H.div ! A.class_ "mdl-card__title output-title" $ do
            H.h2 ! A.class_ "mdl-card__title-text" $ string $ "Response from socket with products: " ++ show products
            H.button ! A.class_ "stop-streaming mdl-button mdl-js-button mdl-button--raised mdl-button--colored" $
                "Stop"
        ...
        H.input ! A.type_ "hidden" ! A.name "initial_message" ! A.value (stringValue . cs $ initialMessage)
        H.script ! A.type_ "text/javascript" ! A.defer "" ! A.src (stringValue $ jsFile "playground/stream.js") $
            return ()
...
renderError err =
    H.div ! A.class_ "output-card demo-card-wide mdl-card mdl-shadow--2dp" $ do
        H.div ! A.class_ "mdl-card__title" $
            H.h2 ! A.class_ "mdl-card__title-text" $ string $ "Response from socket with initial message: " ++ err
