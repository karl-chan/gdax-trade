...
updateBalances :: ExchangeMessage -> FeesConf -> Reader MyAccount (HashMap Currency Balance)
        ...
        (deltaBalance1, deltaBalance2) =
            case msg of
                Open {..} ->
                    case msgSide of
                        Buy ->
                            let holdAmount = realToFrac msgPrice * realToFrac msgRemainingSize
                            in ( zero
                               , DeltaBalance {deltaTotal = 0, deltaAvailable = -holdAmount, deltaHeld = holdAmount})
                        Sell ->
                            let holdAmount = realToFrac msgRemainingSize
                            in ( DeltaBalance {deltaTotal = 0, deltaAvailable = -holdAmount, deltaHeld = holdAmount}
                               , zero)
                Match {..} -> do
                    ...
                    if msgSide == Buy && isMaker || msgSide == Sell && isTaker
                        then let creditAmount = realToFrac msgSize
                                 debitAmount = realToFrac msgPrice * realToFrac msgSize * (1 + percentFee)
                                 releaseAmount = realToFrac msgPrice * realToFrac msgSize
                             in ( DeltaBalance {deltaTotal = creditAmount, deltaAvailable = creditAmount, deltaHeld = 0}
                                , DeltaBalance
                                  {deltaTotal = -debitAmount, deltaAvailable = -debitAmount, deltaHeld = releaseAmount})
                             ...
                             in ( DeltaBalance
                                  {deltaTotal = -debitAmount, deltaAvailable = -debitAmount, deltaHeld = releaseAmount}
                                , DeltaBalance {deltaTotal = creditAmount, deltaAvailable = creditAmount, deltaHeld = 0})
                ChangeLimit {..} ->
                    case msgSide of
                        Buy ->
                            let holdAmount = realToFrac msgPrice * realToFrac (msgNewSize - msgOldSize)
                            in ( zero
                               , DeltaBalance {deltaTotal = 0, deltaAvailable = -holdAmount, deltaHeld = holdAmount})
                        Sell ->
