    ...
    liftIO $ do
        ...
        forkIO $ do
            ...
            let loop series = do
                    ...
                    logDebug $ "New stat: " ++ show stat ++ ". New time series range: " ++ showRange newSeries
                    loop newSeries
            loop initialTS
        return tsFeed
...
initialSeries startTime endTime product = do
    ...
    let intervalLength = granularity * fromIntegral dataLimit
        ...
        tasks = map (\(s, e) -> runReaderT (restSeries s e product) config) intervals
    multiSeries <- liftIO $ throttle parallelism throttleInterval (Just retryGap) tasks
...
insertStat _ _ = error "insertStat only works for Match messages"
...
updateStat _ _ = error "updateStat only works for Match messages"
